...
export class StructuresWSO2Service {
  ...
  async create(dto: CreateStructureDto, token: string): Promise<Structure> {
    try {
      ...
      const displayName = StructureNameHelper.buildPath((dto.parentName) ? [dto.parentName, dto.name] : [dto.name]);
      ...
      const res: AxiosResponse<any> = await axios.post(this._baseUrl, payload, this._getRequestOptions(token));
      return this._mapFromWSO2(res.data);

    } catch (err: any) {
      if (err instanceof BadRequestException) throw err;
      throw new InternalServerErrorException('Error creando estructura en WSO2');
    }
  }
  ...
  async findAll(token: string): Promise<Structure[]> {
    try {
      const res: AxiosResponse<any> = await axios.get(this._baseUrl, this._getRequestOptions(token));
      return (res.data?.Resources ?? []).map((g: any) => this._mapFromWSO2(g));
    } catch (err: any) {
      throw new InternalServerErrorException('Error obteniendo estructuras de WSO2');
    }
  }
  // Buscar estructura por ID
  async findOne(id: string, token: string): Promise<Structure> {
    try {
      const res: AxiosResponse<any> = await axios.get(`${this._baseUrl}/${id}`, this._getRequestOptions(token));
      return this._mapFromWSO2(res.data);
    } catch (err: any) {
      if (err.response?.status === 404) throw new NotFoundException(`Structure ${id} not found`);
      throw new InternalServerErrorException('Error buscando estructura en WSO2');
    }
  }
  ...
  async findOneByName(name: string, token: string): Promise<Structure> {
    try {
      ...
      throw new InternalServerErrorException('Error buscando estructura por nombre en WSO2');
    }
  }
  ...
  async update(id: string, dto: UpdateStructureDto, token: string): Promise<Structure> {
    try {
      ...
      const allStructures = await this.findAll(token);
      const existingNames = allStructures.filter(s => s.id !== id).map(s => s.name);
      StructureNameHelper.ensureUnique(displayName, existingNames);
      ...
      if (err.response?.status === 404) throw new NotFoundException(`Structure ${id} not found`);
      throw new InternalServerErrorException('Error actualizando estructura en WSO2');
    }
  }
  ...
  async remove(id: string, token: string): Promise<void> {
    try {
      ...
      if (err.response?.status === 404) throw new NotFoundException(`Structure ${id} not found`);
      throw new InternalServerErrorException('Error eliminando estructura en WSO2');
    }
  }

  // Mapper de SCIM2 → Entidad de dominio
  private _mapFromWSO2(data: any): Partial<Structure> {
    const parts = data.displayName.split('/');
    return {
      id: data.id,
      name: parts[parts.length - 1],
      parentName: parts.length > 1 ? parts.slice(0, -1).join('/') : null,
      children: [],
      users: []
    } as Structure;
  }
